...
int z, grouped_cars, ungrouped_cars; boolean group_status, group_deleted;
...
List<WebElement> cars_title, cars;
	...
	//	login();
		...
		wait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[@class='active-car-title']"))).click();
		Thread.sleep(1000);
		List<WebElement> total_cars= wait(driver, 10).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//span[@class='car-title']")));
		wait(driver, 10).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//span[@class='active-car-title']"))).click();
		
		if(total_cars.size()>6) 
		{
		...
		try {
		group_status = wait(driver, 15).until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[contains(text(),'Test_Group')]"))).isDisplayed();
		} catch(Exception e) {
		}
		...
		//Edit group
		edit_grp_btn(); Thread.sleep(2000);
		...
		softassert.assertAll();
			...
			System.out.println("Car group option not available in Menu because of"+" "+total_cars.size()+" "+ "car");
		}
	}
	...
	public int grouped_cars() {
		cars_title = wait(driver, 10).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//div[@class='group-container']/div[1]/div[1]")));
		cars = wait(driver, 10).until(ExpectedConditions.visibilityOfAllElementsLocatedBy(By.xpath("//div[@class='group-container']/div[1]/div[2]")));
